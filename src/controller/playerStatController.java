...
//      switch (asc) { // GD = 득실차
//      case 1 :
//         rs = stmt.executeQuery("SELECT PNO, PNAME,TNAME, G_COUNT, GOAL, ASSISTS, SHOTS, SHOTS_ON_GOAL, FOUL, YCARD, RCARD OFFSIDE FROM PLAYERSTAT ORDER BY G_COUNT DESC"); // 승정렬 
//         break;
//      case 2:
...
//      selectByxxx();  득점왕, 도움왕, 슈팅1등, 유효1등,(전체, 특정팀(join))  
      ...
      public static void selectAllAssists() throws SQLException { // 4. 어시스트 전체 1 어시스트 이상 전체 출력!
          ...
          System.out.println(String.format("%-12s | %-7s | %-6s | %-2s", "이름","팀","경기","득점"));
		  System.out.println("------------------------------------------------");
    	  while(rs.next()) {
